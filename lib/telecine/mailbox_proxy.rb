module Telecine
  ...
  class MailboxProxy
    ...
    def inspect
      "#<Telecine::MailboxProxy:0x#{object_id.to_s(16)} #{address}>"
    end
    ...
    def self._load(string)
      ...
      if Telecine.id == node_id
        # If we're on the local node, find the real mailbox
        mailbox = Telecine::Router.find mailbox_id
        ...
        Telecine::MailboxProxy.new(node_id, node_addr, mailbox_id)
      end
    end
  end
end
